...
def create_vision_transformer(model_type: str, pre_trained_weights: bool, num_classes: int, device: str) -> nn.Module:
    ...
    if model_type == "vit_b_16":
        ...
        model = create_model("swin_base_patch4_window7_224", pretrained=pre_trained_weights)
        model.head = nn.Linear(model.head.in_features, num_classes)
    elif model_type == "swin_l":
        model = create_model("swin_large_patch4_window7_224", pretrained=pre_trained_weights)
        model.head = nn.Linear(model.head.in_features, num_classes)
    else:
